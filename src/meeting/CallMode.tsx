...
import { useCopilotAnalysis } from './useCopilot';
import { AUTO_ANALYZE_STAGE_KINDS } from '../knowledge/copilotAnalysis';
...
export function CallMode({ productId, meetingId, navigate }: CallModeProps) {
  ...
  const copilot = useCopilotAnalysis(update);

  const [objectionOpen, setObjectionOpen] = useState(false);
  const [copilotOpen, setCopilotOpen] = useState(false);

  // Só a objeção é um overlay "verdadeiro" nos dois tamanhos de tela — o
  // Copilot vira coluna fixa no desktop (ver meeting.css), então só bloqueia
  // o scroll do body quando é de fato um drawer sobreposto (mobile).
  ...
  // Gatilho A: ao concluir uma etapa importante, analisa automaticamente —
  // dispara ao AVANÇAR de uma etapa cujo kind está em AUTO_ANALYZE_STAGE_KINDS,
  // nunca no carregamento inicial (o ref começa já sincronizado com o índice atual).
  const prevStageIndexRef = useRef(meeting?.currentStageIndex ?? 0);
  useEffect(() => {
    if (!meeting || !product) return;
    const prevIndex = prevStageIndexRef.current;
    const nextIndex = meeting.currentStageIndex;
    prevStageIndexRef.current = nextIndex;
    if (nextIndex <= prevIndex) return; // não dispara ao voltar ou no mount
    const completedStage = product.stages[prevIndex];
    const currentStage = product.stages[nextIndex];
    if (completedStage && currentStage && AUTO_ANALYZE_STAGE_KINDS.includes(completedStage.kind)) {
      copilot.analyze(meeting, 'stage', currentStage.id);
    }
  ...
  return (
    <div className="call-mode">
      <div className="call-main">
        <header className="call-topbar">
          ...
          <div className="call-topbar-actions">
            ...
            <button type="button" className="call-tool-btn copilot-toggle-btn" onClick={() => setCopilotOpen(true)}>Copilot</button>
          </div>
        </header>
      ...
      <CopilotPanel
        ...
        onAnalyzeNow={() => copilot.analyze(meeting, 'manual', stage.id)}
        navigate={navigate}
